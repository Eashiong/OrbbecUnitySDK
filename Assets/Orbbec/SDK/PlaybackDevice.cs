...
namespace Orbbec
{
    ...
    {
        private static Dictionary<IntPtr, PlaybackStatusChangedCallback> _playbackStatusChangedCallbacks = new Dictionary<IntPtr, PlaybackStatusChangedCallback>();
        private NativePlaybackStatusChangedCallback _nativePlaybackStatusChangedCallback;

#if ORBBEC_UNITY
        [AOT.MonoPInvokeCallback(typeof(NativePlaybackStatusChangedCallback))]
#endif
        private static void OnPlaybackStatusChanged(PlaybackStatus status, IntPtr userData)
        {
            _playbackStatusChangedCallbacks.TryGetValue(userData, out PlaybackStatusChangedCallback callback);
            if (callback != null)
            {
                callback(status);
            }
        ...
        */
        ...
        {
            _playbackStatusChangedCallbacks[_handle.Ptr] = callback;
            IntPtr error = IntPtr.Zero;
            obNative.ob_playback_device_set_playback_status_changed_callback(_handle.Ptr, _nativePlaybackStatusChangedCallback, _handle.Ptr, ref error);
            NativeException.HandleError(error);
        }
